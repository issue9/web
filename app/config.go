...
func NewServerOf[T any](name, version string, b server.BuildErrInfoFunc, fsys fs.FS, filename string) (*server.Server, *T, error) {
	if filename == "" {
	...
	opt := &server.Options{
		...
		Logs:           conf.logs,
		ErrInfoBuilder: b, // ErrInfoBuilder 影响输出数据格式，不可以在配置文件中被修改。
