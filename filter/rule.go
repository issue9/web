...
// S 将一组修正数据的函数封装为 [RuleOf]
func S[T any](f ...func(*T)) RuleOf[T] {
...
// V 将验证器函数封装为 [RuleOf]
func V[T any](f func(T) bool, msg localeutil.Stringer) RuleOf[T] {
...
func SS[S ~[]T, T any](f ...func(*T)) RuleOf[S] {
...
func MS[M ~map[K]V, K comparable, V any](v func(*V)) RuleOf[M] {
...
// SV 将验器封装为 [RuleOf] 用以验证切片元素
func SV[S ~[]T, T any](v func(T) bool, msg localeutil.Stringer) RuleOf[S] {
...
// MV 将验器封装为 [RuleOf] 用以验证 map
//
// v 用于验证键名和键值，两者可以有一个是空值，表示不需要验证，但不能都为空；
// msg 表示验证出错时的错误提示；
func MV[M ~map[K]V, K comparable, V any](v func(V) bool, msg localeutil.Stringer) RuleOf[M] {
	return func(name string, m *M) (string, localeutil.Stringer) {
		for key, val := range *m {
			if !v(val) {
