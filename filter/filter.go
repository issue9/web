...
//
// msg 和 validator 组成验证规则；
// sanitize 表示对字断 v 的一些清理，比如去除空白字符等，如果指定了此参数，会在 rule 之前执行；
func New[T any](msg localeutil.LocaleStringer, validator ValidatorFuncOf[T], sanitize ...func(*T)) FilterFuncOf[T] {
	return NewFromRule(NewRule(validator, msg), sanitize...)
}

// NewFromRule 返回生成 FilterFunc 的方法
//
// sanitize 表示对字断 v 的一些清理，比如去除空白字符等，如果指定了此参数，会在 rule 之前执行；
func NewFromRule[T any](rule RulerFuncOf[T], sanitize ...func(*T)) FilterFuncOf[T] {
