...
func (doc *Parser) parseRESTDoc(t *openapi3.T, currPath, title string, lines []string, ln int, filename string) {
	...
	if t.Info != nil {
		doc.l.Log(logger.DocSyntax, localeutil.Phrase("已经存在一个 # restdoc 节点"), filename, ln)
		return
	}
...
LOOP:
	for i, line := range lines {
		...
		case "@media": // @media application/json application/xml
			doc.media = utils.SplitSpace(suffix)
