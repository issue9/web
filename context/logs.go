...
func (ctx *Context) Critical(v ...interface{}) {
	msg := ctx.logMessage(v) + string(utils.TraceStack(nil, 2))
	logs.CRITICAL().Output(2, msg)
}

// Criticalf 相当于调用了 logs.Criticalf，外加一些调用者的详细信息
func (ctx *Context) Criticalf(format string, v ...interface{}) {
	msg := ctx.logMessagef(format, v) + string(utils.TraceStack(nil, 2))
	logs.CRITICAL().Output(2, msg)
}

// Error 相当于调用了 logs.Error，外加一些调用者的详细信息
func (ctx *Context) Error(v ...interface{}) {
	msg := ctx.logMessage(v) + string(utils.TraceStack(nil, 2))
	logs.ERROR().Output(2, msg)
}
