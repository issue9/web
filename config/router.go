...
type CORS struct {
	...
	//
	// 如果此值为空，表示不启用跨域的相关设置。
	AllowedOrigins []string `yaml:"origins,omitempty" json:"origins,omitempty" xml:"origins,omitempty"`

	// AllowedHeaders 对应 Access-Control-Allow-Headers
	//
	// 可以包含 *，表示可以是任意值，其它值将不再启作用。
	AllowedHeaders []string `yaml:"allowedHeaders,omitempty" json:"allowedHeaders,omitempty" xml:"allowedHeader,omitempty"`
